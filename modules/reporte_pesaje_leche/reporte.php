...
if(isset($_POST['generar_reporte_leche']))
{
	...
	// ENCABEZADOS
	fputcsv($salida, array('Codigo', 'Animal', 'Empleado', 'Fecha', 'Peso mañana','Peso tarde','Peso'));
	// QUERY PARA CREAR EL REPORTE

	$query=mysqli_query($mysqli,"SELECT (p.id)as id,(a.nombre)as animal,(e.nombre)as empleado,(p.fecha)as 
										fecha,(p.peso_am)as peso_am,(p.peso_pm)as peso_pm,(p.peso)as peso
	...
	while($filaR= mysqli_fetch_assoc($query)){
		fputcsv($salida, array($filaR['id'], $filaR['animal'], $filaR['empleado'], $filaR['fecha'], $filaR['peso_am'],$filaR['peso_pm'],$filaR['peso']));
	}
